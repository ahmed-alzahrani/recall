...
class ChunkingService {
    ...
    fun chunkDocument(pages: List<PageText>): List<ChunkData> {
        val sentences = parsePagesIntoSentences(pages)
        if (sentences.isEmpty()) {
            return emptyList()
        }
        return buildChunksFromSentences(sentences)
    }

    private fun parsePagesIntoSentences(pages: List<PageText>): List<SentenceWithPage> {
    ...
    private fun buildChunksFromSentences(sentences: List<SentenceWithPage>): List<ChunkData> {
        ...
        for (i in sentences.indices) {
            ...
            if (wouldExceed && currentChunkWords >= minChunkWords) {
                ...
                val overlapSentences = getOverlapSentences(currentChunkSentences, overlapWords)
    ...
    private fun getOverlapSentences(sentences: List<String>, targetWords: Int): List<String> {
        var wordCount = 0
    ...
    private data class SentenceWithPage(val text: String, val pageNumber: Int, val wordCount: Int)
}
